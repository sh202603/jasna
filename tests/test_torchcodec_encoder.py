...
from jasna.media import VideoMetadata  # noqa: E402
...
class TestRejection:
    def _enc(self, **ov):
        ...
        meta_ov = {k: ov.pop(k) for k in ("is_10bit", "color_space", "color_range") if k in ov}
        if meta_ov:
            kw["metadata"] = _meta(**meta_ov)
        kw.update(ov)
        return TorchcodecVideoEncoder(**kw)

    def test_10bit_source_rejected(self):
    ...
    def test_no_audio_mux_rejected(self):
        from jasna.media.torchcodec_encoder import TorchcodecEncodeUnsupported
        with pytest.raises(TorchcodecEncodeUnsupported):
            self._enc(mux_audio=False)

    def test_changed_output_fps_rejected(self):
        from jasna.media.torchcodec_encoder import TorchcodecEncodeUnsupported
        with pytest.raises(TorchcodecEncodeUnsupported):
            self._enc(output_fps=Fraction(48, 1))
