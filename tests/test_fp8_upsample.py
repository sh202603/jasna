...
def test_gate_returns_none_without_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _try_create_fp8_upsample(MagicMock(), torch.device("cuda"), fp16=True, max_clip_size=90) is None


def test_gate_returns_none_with_fp32(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("JASNA_FP8_RECON", "1")
    assert _try_create_fp8_upsample(MagicMock(), torch.device("cuda"), fp16=False, max_clip_size=90) is None


def test_gate_returns_none_with_cpu_device(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("JASNA_FP8_RECON", "1")
    assert _try_create_fp8_upsample(MagicMock(), torch.device("cpu"), fp16=True, max_clip_size=90) is None
...
def test_gate_falls_back_on_constructor_error(
    ...
    with caplog.at_level("WARNING"):
        result = _try_create_fp8_upsample(MagicMock(), torch.device("cuda"), fp16=True, max_clip_size=90)
...
def test_create_split_forward_uses_fp8_and_skips_trt_upsample(
    ...
    def _fake_load(model_weights_path, device, fp16, max_clip_size=60, load_upsample=True):
