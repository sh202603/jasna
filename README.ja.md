...
処理速度への影響は計測できる範囲では生じません。既定 off（従来挙動）。GUI ではエンコード設定のトグルで同じ機能を使えます。`--segments`・`--stream`・torchcodec エンコードバックエンドでは警告を出して通常の MP4 出力にフォールバックします。詳細: [docs/ja/fmp4.md](docs/ja/fmp4.md)。
