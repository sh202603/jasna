...
このブランチが upstream（`main`）に対して加えている変更の要約です。`modi` は upstream main `d7a99bd`（`v0.8.1` タグ + 23 コミット）にリベース済み。

- ブランチ: `modi`（fork `https://github.com/sh202603/jasna.git`）
- バージョン: `0.8.1+modi`（upstream main `d7a99bd` ベース = `v0.8.1` + リリース後 23 コミット）
