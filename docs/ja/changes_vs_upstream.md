...
一次の BasicVSR++ が大きなモザイク領域・接写・4K でぼやけさせるテクスチャを、拡散 one-step VSR の [FlashVSR](https://github.com/OpenImagingLab/FlashVSR)（fork [`sh202603/FlashVSR_plus`](https://github.com/sh202603/FlashVSR_plus)。[`lihaoyun6/FlashVSR_plus`](https://github.com/lihaoyun6/FlashVSR_plus) をさらに fork したもの）で補う二次復元。256px の一次復元クロップを 1024px（4x、モデルネイティブ）または `--flashvsr-scale 2` で 512px へ拡大して再 blend する（出力解像度は変わらない）。FlashVSR は独自ライセンスのサードパーティモデルで、checkout・重み・専用 venv は利用者が用意し `--flashvsr-repo` で渡す（jasna のサポーターモデルとは無関係、同梱なし）。FlashVSR venv の基底 Python は **Python 開発ヘッダ必須**（Triton の Sparse_SageAttention カーネルを実行時 JIT するため。uv-managed standalone Python か -dev パッケージ入りの system Python）。**2 つのモード**があり、どちらもサポートする。
...
- **`--secondary-restoration flashvsr-inline`（inline 単一パス）**: FlashVSR を通常のストリーミングパイプラインに二次復元として挟む。**中間ファイル・ディスクゲート・二重 encode が無い**。FlashVSR の **tiny-long**（O(1)、フレーム数非依存で定常 ~11.9 GB）を使い、fp8-recon（§8）で ~1.6 GB に絞った一次と**同時常駐**させて単一パスを実現する。同期 `SecondaryRestorer` が FlashVSR venv worker を resident spawn し、length-prefixed の uint8 BGR プロトコル（worker は lada-ex の `flashvsr_worker.py` と**逐語同一**に保ち、RGB 反転は jasna 側アダプタで行う。SeedVR2 worker と同じ方針）で 256px→256×scale px をやり取りする（`imageio.get_writer` を差し替えロスレスにテンソル捕獲、小 clip は next_8n5 パディングで吸収し厳密に T 枚返す）。offline と同じく frame-gen off を強制し、同時常駐予算のため **fp8-recon を自動有効化**する。VRAM 天井対策として、DiT 推論を横短冊に分割して羽根合成する **`--flashvsr-tiles {1..4}`**（strip tiled-dit、inline 専用。`2` で attn マスク 0.39x / 壁時計 ~1.25x）を持つ。**16 GB カード + tiny-long の修正を含む checkout（推奨 fork、またはパッチを当てた上流）が前提**。実装 `jasna/restorer/flashvsr_inline_secondary_restorer.py` + `flashvsr_inline_worker.py`。seedvr2 一次復元（§14）とは排他（常駐 worker 2 つで 16 GB 超過、起動時エラー）で、その構成はオフラインモードで組む。

- **v2（lada-ex rev 3 の移植、2026-09）**: (1) **`--flashvsr-scale {2,4}`**（既定 4、両モード共通）。scale 2 は 512px 処理で、逐語同一の worker を持つ lada-ex 実測（480p）では scale 4 + tiles 2 に対し壁時計約 1/5・GPU 全体ピーク約 4 GB 減、flow-warping error 比 1.128（ゲート ≤1.2）。jasna のブレンドは復元フレームの実寸から幾何を導くため、ブレンド側の変更は無い。strip の丸めは `(128 // scale)` 倍数へ一般化。(2) **出力クロップの色補正を常時適用**（両モード）: 一次出力の bicubic 拡大を参照に wavelet 再構成（高周波 = FlashVSR、低周波 = 参照）で合わせ、合成領域と周囲の色調差を除く。lada-ex 実測で二次が変えた画素の median |Δmean| 5.28 → 0.34。上流の `pipe(color_fix=True)` は裸の `except: pass` で失敗を黙殺するため使わず、inline は worker の合成後に、オフラインは Phase 2 driver が worker から同じ関数を読み込んで適用する。CLI フラグは無く、`JASNA_FLASHVSR_COLOR_FIX=adain|wavelet|none` は検証専用の上書き。(3) オフラインのディスク見積りは scale に追従（scale 2 で 1/4）。
- **v2.1（2026-09-19）**: (1) **clip 32 上限を撤廃**: inline は `--max-clip-size`（既定 90）をそのまま使う（tiny-long は clip 長に対して VRAM 平坦で、v0.9.1 の固定バッチ化以降は一次側の clip 長コストも FlashVSR なしと同じ）。offline の `--flashvsr-max-clip-frames` 既定は 90 へ（tiny モードの Phase 2 ピークは 32 → 90 で両 scale とも平坦と実測）。clip 32 では overlap 8 に対し 1 clip の前進が 16 フレームしかなく DiT フレーム 1.8 倍・呼び出し 3 倍だったため、clip 90 で FlashVSR 時間は両モード・両 scale でほぼ半減（inline scale 2 / tiles 1: 480p 163 → 100 秒、1080p 274 → 146 秒。scale 4 / tiles 2: 841 → 479 秒。offline scale 4 の Phase 2: 864 → ~430 秒）、VRAM 増は一次のキュー分 +0.1〜0.7 GB。(2) **worker が clip ごとの出力を検査**: FlashVSR_plus の tiny-long は chunk ループの例外を握り潰す（traceback を出して False を返す）ため、VRAM 天井での clip 途中 OOM が途中までの clip として返り、worker が残りを最後のフレームの複製で埋めて静止したクロップが動く映像に合成されていた（残像。1080p / scale 4 / tiles 1 / clip 90 で 57 clip 中 32 回）。worker は `pipe()` の戻り値と枚数を検査し、キャッシュ解放後に 1 回リトライ、再失敗なら clip エラーで停止する。したがって scale 4 の tiles 無しは 16 GB では使えない（clip 長に関わらず 480p でも 1080p でも天井）。`--flashvsr-tiles 2` を使う（480p 13.7 GB、1080p 14.7 GB、offload なし）。
- **v2.2（2026-09-23）**: (1) **`--flashvsr-accel`**（既定 off、両モード共通）: fork `sh202603/FlashVSR_plus` の高速化（LQ projector の FP8 畳み込み、DiT の FP8 Linear、DiT の融合カーネル。fork の `--accel`）を、worker / Phase 2 driver の環境変数 `FLASHVSR_ACCEL=1` で有効にする。fork の計測で FlashVSR 1.37〜1.41 倍速、ピーク −1.4 GiB。RTX 40 系未満・fp16・`-v 10`・上流の checkout では部品ごとに自動で標準の処理へ戻り、全部品が外れた出力はフラグなしとビット一致。`--no-flashvsr-accel` はシェルの `FLASHVSR_ACCEL` も取り除く。(2) worker の stdout は捨てられるので、worker は起動時の判定結果（有効な部品と fork のログ行）を ready ハンドシェイクで、実行中に外れた部品を clip の応答ヘッダで jasna に返し、jasna がログに出す。warmup は実際の clip 形状（strip を含む）で行う。(3) Phase 2 driver は、clip の途中で部品が外れたとき（tiny パイプラインは例外をそのまま上げる）に 1 回だけやり直す。(4) セットアップ手順を fork 優先に改め、`uv sync` で依存を入れる形にした（fork は tiny-long の修正を含むのでパッチ不要）。
- **v2.3（2026-09-23）**: (1) **`--flashvsr-lora`**（inline 専用、既定なし）: worker の起動時に、Lada の LoRA を FlashVSR の DiT へ bf16 の低ランクアダプタとして適用する。公開している `lada_flashvsr_secondary_lora_v1.pt`（[sh202603/lada-seedvr2-lora](https://huggingface.co/sh202603/lada-seedvr2-lora)）は、素の FlashVSR の中帯域の過鮮鋭を抑え（4〜8 px 帯で約 −30%）、拡大の小さい領域で過剰な粒を周囲の肌の水準へ戻し、ちらつきを少し減らす。代わりに最も細かい粒の一部を失う。`--flashvsr-accel` と併用できる（アダプタは FP8 の Linear の横と融合 FP8 FFN の中に置く）。実行中に `fp8_dit` が失敗するとフォールバックでアダプタが外れるため、その部品を切って worker を起こし直し、clip をやり直す。オフラインモードではこのフラグを拒否する（主な用途の SeedVR2 一次では、この LoRA が FlashVSR の足す粒の大半を消すため）。worker は再び lada-ex と逐語同一になった。(2) inline の restorer が、求めに応じて worker を起こし直すようになり、実行中に外れた高速化の部品は起こし直した後も切ったままにする。(3) 修正: `restore()` が CPU 上の float32 入力テンソルをその場で書き換えていた問題を直した。 Linux 実機（RTX 5080、1080p 実素材 6242 フレーム、inline scale 2 + `--flashvsr-accel`）で確認: LoRA の有無とも出力フレーム数 = 入力、目視 A/B で毛の過鮮鋭な見え方が抑えられた。Windows 実機（RTX 5060 Ti、同じ素材、inline scale 2 / tiles 1）でも確認: 高速化の有無とも出力フレーム数 = 入力、LoRA のコストは FlashVSR の処理時間 1.05 倍、壁時計 1.06 倍、目視 A/B で同じ効果が高速化の有無によらず出た。

**tiny-long パッチ（同梱）**: inline が使う tiny-long は fork の実装が第 2 チャンクで壊れる既知バグ（`8192 vs 4096` エラー）があり、2 箇所のチャンク跨ぎキャッシュ clear（per-chunk の `LQ_proj_in.clear_cache()` / `TCDecoder.clean_mem()`）を無効化する修正が要る（ループ前の一度きりのリセットは残す）。上流 `lihaoyun6/FlashVSR_plus` は**不活性**のため上流化せず、修正は fork `sh202603/FlashVSR_plus` に取り込んだ（推奨）。上流の checkout 向けにパッチを [`patches/flashvsr_plus_tinylong_multichunk_fix.patch`](../../patches/flashvsr_plus_tinylong_multichunk_fix.patch) に同梱（利用者が checkout へ `git apply`）。restorer は起動時に checkout がパッチ済みかを検査し、修正が無ければ明示エラーで停止して fork・パッチ・オフライン（tiny、無パッチ可）を案内する。これで唯一同梱されるパッチは mmengine 用（§1）と本パッチの 2 つになる。
...
**検証**: ユニット `tests/test_flashvsr_{offline,inline}.py` + `test_main.py`（GPU 無し CI、full suite リグレッション 0。v2 では strip 幾何の両 scale、BGR wire の往復反転を片側ずつ検出する stub、色補正 primitives の CPU テストを追加）。v2.1 の実機（Linux、480p の実素材 4930 フレーム、clip 90）: inline scale 2 / tiles 1 は 113 秒・GPU 全体ピーク 10.1 GB で完走（scale 4 / tiles 2 は 491 秒・13.7 GB。1080p は 156 秒・11.3 GB、offload なし）、offline scale 2 は 194 秒・7.8 GB（scale 4 は 477 秒・13.2 GB・bundle 9.7 GB）。色補正ゲート（二次が変えた画素の median |Δmean|、一次のみ出力比）は通過（clip 90 で none 1.73 → wavelet 0.25。clip 32 時点は inline 1.76 → 0.24、offline 1.61 → 0.24）。移植前 worker との A/B で FlashVSR 時間は同一、色補正自体のコストは約 6〜7%。実機（RTX 5080 sm120）: オフライン/inline とも E2E 完走・出力フレーム数 = 入力。Linux では inline は 852x480 で combined VRAM peak 14780 MiB、A/B（primary-only 比）でモザイク領域のみが変化することを確認。1080p は物理天井際（~15.8 GB）まで上がり、`vram_offloader`（キューフレームを RAM へ退避）と worker の `expandable_segments` が圧を吸収して完走する。Windows（同カード、`expandable_segments` 非対応）でもフル長 480p/1080p の E2E を確認: 1080p は tiles `1` で GPU 全体ピーク 15.9 GB（天井まで <0.4 GB）、`--flashvsr-tiles 2` で 14.2 GB / 壁時計 +25% となり、短冊境界のシーム（バンディング、色調ずれ）は不検出。Windows 16 GB の 1080p 常用は tiles `2` を推奨。設計と全計測は `docs/{ja,en}/flashvsr.md`。
...
`--restoration-model-name seedvr2` は、一次復元器 BasicVSR++ を SeedVR2 3B（one-step SR diffusion）+ 自作 LoRA（モザイクをセル平均化劣化として除去することを学習。rank 16、約 90 MB、[sh202603/lada-seedvr2-lora](https://huggingface.co/sh202603/lada-seedvr2-lora) で公開、AGPL-3.0）に差し替える。検出された 256px クロップを scale=1 のまま diffusion forward に通し、パイプラインの他の部分（検出、クロップ抽出、blend、encode）は不変。既定の `basicvsrpp` 経路は bit 不変（basicvsrpp 実行の復号出力が機能追加前のツリーと全フレーム一致することを実測確認）。本フォークの lada-ex 実装からの移植で、worker ファイルは lada-ex と**逐語同一**に保つ（上流 numz repo の互換性破壊時に片方で直して diff コピーするため）。位置づけ: この LoRA は低解像度で劣化の大きい素材に最も強い（BasicVSR++ が均す情報の少ない入力からも diffusion の事前分布がディテールを再構成するため）。生成系の出力は原信号の復元でなく「ありそうなディテール」の付与であり BasicVSR++ の完全な置き換えではないため、一次復元器は素材に応じて使い分ける（既定は従来どおり `basicvsrpp`）。

プロセスモデル: 利用者が用意する [ComfyUI-SeedVR2_VideoUpscaler](https://github.com/numz/ComfyUI-SeedVR2_VideoUpscaler) venv 内の常駐 worker（`jasna/restorer/seedvr2_lora_worker.py`、jasna import ゼロ）と、stdin/stdout 上の JSON ヘッダ + 生 uint8 BGR で 1 クリップ入出力を往復する。死亡時は respawn + 同一クリップ 1 回リトライ、素通しフォールバックなし（クリップのスキップはモザイク残存を意味するため）。worker 内部は peft による LoRA 注入（lada-ex 由来の bitsandbytes スタブ除去と fp32 adapter cast 込み）、33f/stride24 のスライディングウィンドウを GPU 常駐の float32 アキュムレータへクロスフェード合成して最後に 1 回だけ量子化（CPU への転送はクリップ毎 1 回。旧来のフレーム毎 CPU 合成と bit 一致）、入力モザイククロップ参照のクリップ一括 LAB 色転写、総 VRAM で自動切替するクリップ毎 `empty_cache`（`--seedvr2-empty-cache`。同居する検出/デコード側プロセスへの VRAM 返却で、総 VRAM 20GiB 未満では OOM 保険・以上では純オーバーヘッドのため `auto` は未満のみ解放）。jasna 固有の適応は 3 点: この復元器ではクロップのパディングを reflect でなくゼロ埋めにする（`prepare_crops_for_restoration` へ `pad_mode` を配管。LoRA の訓練分布がゼロパディングのため）、ワイヤは lada ネイティブの BGR のまま親アダプタ側で RGB 変換する、blend は一切変更不要（jasna の `create_blend_mask` は最初から mask 限定 + 膨張/フォールオフ構造で、lada が別途追加した mask 限定版と同型のため）。A/B 目視ゲートから 4 点目の適応が出た: diffusion の VAE/DiT はゼロパッドの黒を有効領域内 2〜3px へ滲ませ、blend mask の膨張+フォールオフが enlarged bbox の境界余白を越えるため、bbox 縁に暗線（最大 -145 luma）として合成されていた。対策として各クロップ有効領域のパッド隣接 4px 帯を線形ランプで入力へ戻す（`_revert_valid_edges`。復元器の `edge_revert_px` 属性でゲートし BasicVSR++ は 0 のため当該経路は bit 不変のまま）。最悪クリップの実測で暗線指標は -176 から bvpp 基準まで解消（縦は完全一致。横 1 点の残存はコンテンツレベルで目視判定待ち）。さらに 5 点目の適応として、残存していた bbox 輪郭の視認を除去した: 1-step diffusion はクロップ全体を再合成するため、blend フォールオフ環帯には低周波がずれ（+2〜6 luma のハロー）粒状感も半減したコンテンツが合成されていた。検出マスクの信頼ゾーン（マスク + blend の dilation）の外側をすべて入力へ戻し、revert の重みには blend mask 自身のフォールオフを流用する（`_revert_outside_mask`。復元器の `revert_outside_mask` 属性でゲート）。検出漏れの保険帯は完全に復元されたまま、フォールオフ帯は実質原画を合成する。大型モザイクでは信頼ゾーンがクロップ全体を覆うため、クロップ境界ランプ（フレーム換算 ~20px = bbox 拡張が保証するマスクフリー余白。フレーム端接触辺は除外）で全縁のコンテンツを入力へ収束させる。seedvr2 選択時は BasicVSR++ のエンジンコンパイル自体をスキップする。制約: VR モード、`--frame-gen`、`flashvsr-inline` 併用は起動時拒否。TVAI と `--stream` は警告。オフライン `flashvsr` は合成可（Phase 分離で VRAM が重ならないため、SeedVR2 復元 + FlashVSR 4x が最高品質構成になる）。
...
**検証**（Linux、RTX 5080 16 GB）: 実測と色補正ゲートは `docs/{ja,en}/swiftvr.md`。目視 A/B（利用者、480p と 1080p、FlashVSR inline と比較）: scale 4 の肌理とディテールは FlashVSR と同等、scale 2 は時間安定性が scale 4 より弱く 1080p でははっきり視認できる（既定の scale 4 を推奨）。crop 単位の切り分けで、scale 2 の揺れはチャンク境界、FP8、窓の大きさではなく 512px での DiT の生成そのものに由来し、生成を弱める調整は肌理も同じだけ減らすと分かったので、scale 2 は据え置いた（`docs/ja/swiftvr.md` の「scale 2 の時間安定性」）。Windows は未検証。
