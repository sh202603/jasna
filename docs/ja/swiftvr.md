...
attention の構造が違う。動作はするが品質は scale 4 と同じゲートで別途確認する
(「[検証](#検証)」)。どちらでも出力動画の解像度は変わらない。
...
- scale 4 は 1080p で 14.0 GB と天井まで 2 GB 強なので、常駐の多い環境では
  scale 2 を使う。短冊分割は無い。
...
- **時間方向の変化**(二次が変更した領域内の隣接フレーム差の、一次のみ出力に対する比。
  flow 補償なしの粗い代理指標): scale 4 で 1.021、scale 2 で 1.056、補正なしで 1.030
  (目安 1.2 以下)。
- 目視 A/B は利用者が行う。比較用の FlashVSR 出力は同日に同じ素材で作った。
