...
**補足: FP8 復元バックエンドに追加のインストール手順は不要。** 依存 `nvidia-cudnn-frontend` と（Windows では）`triton-windows` は `pyproject.toml` の通常依存で、上記コマンドで一緒に入る。cuDNN ランタイム（9.17 以上）は torch cu130 wheel に同梱済み。機能は実行時 opt-in（`--fp8-recon`、FP8 対応 GPU sm89 以上が必要）で、使えない環境では TensorRT エンジンにフォールバックする。compute glue は triton-windows 経由でコンパイルされ、inductor が動かない環境では警告を出して eager に降格する。姉妹実装の lada-ex は Windows で検証済み（当時は eager glue）だが、本ブランチの計測は Linux のみ。詳細は [FP8_RECON_ja.md](FP8_RECON_ja.md)。
