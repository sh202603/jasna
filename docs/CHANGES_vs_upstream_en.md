...
>
> **Verification status** (2026-07-18 GPU session complete): pytest with GPU: **1678 passed / 9 failed + 1 collection error** — every failure is missing-protection (unet4x/sd15/video_session) or Japanese-locale environment-dependent (one segment_editor layout test; CJK font line height + 1.33 scaling), i.e. **no rebase regressions**. All CLI runs completed: native 480p (10661-frame parity, bt709 tags, audio copy), torchcodec decode (4K), forced torchcodec encode (bt709+tv tags, lossless audio copy, faststart moov-first verified via ffprobe), `--segments` (matching h264 output, full duration kept), `--frame-gen 2x` (30→60fps, (N−1)×2+1 frames), `--fp8-recon` (cuDNN FP8 path confirmed active), `--retarget-high-fps` (60→30fps halving), FlashVSR offline (frame parity) plus its guard rejections (retarget/segments/VR), and GUI startup (interactive checks still pending). TRT sub-engines recompiled on the v0.8.0 default b90 profile. This session also **found and fixed the NVENC pitch-alignment bug (§10)**. Dev-environment note: a source-built PyAV wheel **must link an FFmpeg 8 built with nv-codec-headers 12.2+** (distro FFmpeg lacks `lookahead_level`, so every encode dies with a startup `ValueError`; the working recipe is linking BtbN shared builds and vendoring via auditwheel).
