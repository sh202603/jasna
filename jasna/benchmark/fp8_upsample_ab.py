...
# Matches the pipeline default (--max-clip-size 90) and the on-disk
# upsample_dyn_b90 engine profile.
MAX_CLIP = 90
WARMUP = 10
RUNS = 50
LATENCY_TS = (10, 30, 60, 90)
CAPTURE_VIDEO = Path("assets/test_clip1_1080p.mp4")

GATE_SPEEDUP_T90 = 1.4
...
def benchmark_fp8_upsample_ab(
    ...
    **_: object,
    ...
    paths = get_basicvsrpp_sub_engine_paths(path, True, MAX_CLIP)
    if not all(os.path.isfile(p) for p in paths.values()):
        print(f"fp8_upsample_ab: b{MAX_CLIP} sub-engines missing "
    ...
    try:
        ...
        split = create_split_forward(
            ...
            fp16=True, max_clip_size=MAX_CLIP,
        )
    finally:
