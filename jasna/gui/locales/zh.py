...
ZH = {
        ...
        "segments_ab_compare": "A/B 对比…",
        "segments_ab_compare_hint": "在当前帧上并排对比两组检测/修复模型组合",
        "ab_title": "A/B 模型对比",
        "ab_view_restored": "修复",
        "ab_view_detection": "检测",
        "ab_view_clip": "片段",
        "ab_run": "运行",
        ...
        "ab_stale_hint": "（未更新）",
        "ab_checkpoint_missing_ema": "检查点 {name} 缺少 EMA 权重（generator_ema.*），将以随机权重运行，已拒绝",
        "ab_no_detection": "此帧没有检测结果",
        "ab_score": "检测分数：{score}",
        "ab_step_second_back": "后退 1 秒",
        ...
        "ab_compiling": "正在编译引擎…",
        "ab_compile_done": "引擎编译完成",
        "ab_compile_failed": "引擎编译失败：{message}",
        ...
        "frame_gen_model_path": "补帧模型",
        "frame_gen_model_path_placeholder": "留空则使用 model_weights/rife.pth",
        "tip_frame_gen_model_path": "RIFE 权重的可选路径（推荐 TorchScript .pth）。\n留空则使用 model_weights 文件夹中的 rife.pth。\n默认：空",
        "dialog_select_frame_gen_model": "选择 RIFE 模型检查点",
        "video_backend": "视频后端",
        "tip_video_backend": "解码/编码后端（实验性）。\nNative：vali + PyNvVideoCodec（默认，当前行为）。\nAuto：在可用时使用 torchcodec，否则回退到 Native。\nTorchCodec：强制使用 torchcodec —— 仅支持 8 位；无法满足请求时报错（如 10 位、补帧）。\n默认：Native",
        "fp8_recon": "FP8 修复（实验性）",
        "tip_fp8_recon": "将 BasicVSR++ 上采样阶段以 cuDNN FP8 卷积运行：该阶段更快，显存峰值降低约 1-2 GB。\n需要支持 FP8 的 GPU（RTX 40 系或更新/sm89）并开启 FP16 模式。任何失败都会回退到 TensorRT 引擎。\n默认：关",
        "quality_cq": "质量 (CQ)",
        "custom_args": "自定义参数",
