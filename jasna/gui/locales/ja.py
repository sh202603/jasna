...
JA = {
        ...
        "fmp4": "フラグメント化 MP4（処理中に再生可能）",
        "tip_fmp4": "MP4/MOV 出力をフラグメント化 MP4（fragmented MP4）で書き出します。処理の進行中でもファイルを開いて再生（音声付き）でき、処理を中断してもファイルは再生可能なまま残ります。処理完了までは一部プレイヤーで総時間が不完全に表示されることがあり、非常に古いプレイヤーはフラグメント化 MP4 に非対応の場合があります。MKV 出力とセグメントジョブには影響しません。",
        ...
        "frame_gen_model_path": "生成モデル",
        "frame_gen_model_path_placeholder": "空欄で model_weights/rife.pth を使用",
        "tip_frame_gen_model_path": "RIFE重みへの任意パス（TorchScript .pth 推奨）。\n空欄の場合は model_weights フォルダの rife.pth を使用します。\n既定: 空欄",
        "dialog_select_frame_gen_model": "RIFEモデルのチェックポイントを選択",
        "video_backend": "ビデオバックエンド",
        "tip_video_backend": "デコード/エンコードのバックエンド（実験的）。\nNative: vali + PyNvVideoCodec（既定、従来の動作）。\nAuto: 利用可能な場合は torchcodec を使い、それ以外は Native にフォールバック。\nTorchCodec: torchcodec を強制 — 8bitのみ。要求を満たせない場合（10bit、フレーム生成など）はエラー。\n既定: Native",
        "fp8_recon": "FP8復元（実験的）",
        "tip_fp8_recon": "BasicVSR++のアップサンプル段をcuDNN FP8畳み込みで実行します。処理が高速化し、VRAMピークが約1〜2GB下がります。\nFP8対応GPU（RTX 40シリーズ以降/sm89）とFP16モードONが必要。失敗時はTensorRTエンジンにフォールバックします。\n既定: オフ",
        "quality_cq": "品質 (CQ)",
        "custom_args": "カスタム引数",
        ...
        "tip_vr_mode": "サイドバイサイド VR180 の処理方法を設定します。\n\n自動は、高さが 1080 ピクセルを超える正確な 2:1 フレーム、信頼できるスタジオ名トークン、または互換性のある空間メタデータがある場合に有効になります。SBS は左右の目を別々に検出・修復します。SBS + 魚眼は検出精度向上のため各目を再投影し、修復された領域だけを元の投影へ合成し直します。\n\n区間エディターは左目のみ表示しますが、スキャンと書き出しは両目を処理します。\nデフォルト：自動",
