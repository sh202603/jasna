...
EN = {
        ...
        "segments_ab_compare": "A/B compare…",
        "segments_ab_compare_hint": "Compare two detection/restoration model combinations side by side on the current frame",
        "ab_title": "A/B model comparison",
        "ab_view_restored": "Restored",
        "ab_view_detection": "Detection",
        "ab_view_clip": "Clip",
        "ab_run": "Run",
        ...
        "ab_stale_hint": "(stale)",
        "ab_checkpoint_missing_ema": "Checkpoint {name} has no EMA weights (generator_ema.*) and would run with random weights; refused",
        "ab_no_detection": "No detection at this frame",
        "ab_score": "Detection score: {score}",
        "ab_step_second_back": "1 second back",
        ...
        "ab_compile_done": "Engine compilation finished",
        "ab_compile_failed": "Engine compilation failed: {message}",
        ...
        "frame_gen_model_path": "FrameGen Model",
        "frame_gen_model_path_placeholder": "Leave empty to use model_weights/rife.pth",
        "tip_frame_gen_model_path": "Optional path to RIFE weights (TorchScript .pth recommended).\nLeave empty to use rife.pth in the model_weights folder.\nDefault: empty",
        "dialog_select_frame_gen_model": "Select RIFE model checkpoint",
        "video_backend": "Video Backend",
        "tip_video_backend": "Decode/encode backend (experimental).\nNative: vali + PyNvVideoCodec (default, current behavior).\nAuto: use torchcodec where eligible, otherwise fall back to Native.\nTorchCodec: force torchcodec — 8-bit only; errors when the request cannot be satisfied (e.g. 10-bit, frame generation).\nDefault: Native",
        "fp8_recon": "FP8 Restoration (experimental)",
        "tip_fp8_recon": "Runs the BasicVSR++ upsample stage as cuDNN FP8 convolutions: faster stage and ~1-2 GB lower peak VRAM.\nRequires an FP8-capable GPU (RTX 40-series or newer / sm89) and FP16 Mode ON. Falls back to the TensorRT engine on any failure.\nDefault: OFF",
