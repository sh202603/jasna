...
KO = {
        ...
        "segments_ab_compare": "A/B 비교…",
        "segments_ab_compare_hint": "감지/복원 모델의 두 조합을 현재 프레임에서 좌우로 비교합니다",
        "ab_title": "A/B 모델 비교",
        "ab_view_restored": "복원",
        "ab_view_detection": "감지",
        "ab_view_clip": "클립",
        "ab_run": "실행",
        "ab_detection_model": "감지 모델",
        "ab_restoration_model": "복원 모델",
        "ab_badge_trt": "TRT",
        "ab_badge_trt_rtx": "TRT-RTX",
        "ab_badge_pytorch": "PyTorch",
        "ab_badge_pytorch_hint": "이 체크포인트에는 TensorRT 엔진이 없어 느린 fp32 PyTorch 경로로 실행됩니다. TRT 대비 미세한 품질 차이가 있을 수 있습니다",
        "ab_stale_hint": "(미반영)",
        "ab_checkpoint_missing_ema": "체크포인트 {name}에는 EMA 가중치(generator_ema.*)가 없어 무작위 가중치로 실행되므로 거부했습니다",
        "ab_no_detection": "이 프레임에 감지가 없습니다",
        "ab_score": "감지 점수: {score}",
        "ab_step_second_back": "1초 뒤로",
        ...
        "ab_compile_done": "엔진 컴파일이 완료되었습니다",
        "ab_compile_failed": "엔진 컴파일 실패: {message}",
        ...
        "frame_gen_model_path": "생성 모델",
        "frame_gen_model_path_placeholder": "비워 두면 model_weights/rife.pth 사용",
        "tip_frame_gen_model_path": "RIFE 가중치의 선택적 경로(TorchScript .pth 권장).\n비워 두면 model_weights 폴더의 rife.pth를 사용합니다.\n기본값: 비움",
        "dialog_select_frame_gen_model": "RIFE 모델 체크포인트 선택",
        "video_backend": "비디오 백엔드",
        "tip_video_backend": "디코드/인코드 백엔드(실험적).\nNative: vali + PyNvVideoCodec(기본값, 기존 동작).\nAuto: 가능하면 torchcodec을 사용하고, 아니면 Native로 폴백.\nTorchCodec: torchcodec 강제 — 8비트 전용. 요청을 충족할 수 없으면(10비트, 프레임 생성 등) 오류가 발생합니다.\n기본값: Native",
        "fp8_recon": "FP8 복원(실험적)",
        "tip_fp8_recon": "BasicVSR++ 업샘플 단계를 cuDNN FP8 컨볼루션으로 실행합니다. 단계가 빨라지고 VRAM 피크가 약 1~2GB 낮아집니다.\nFP8 지원 GPU(RTX 40 시리즈 이상/sm89)와 FP16 모드 ON이 필요합니다. 실패 시 TensorRT 엔진으로 폴백합니다.\n기본값: 끔",
        "quality_cq": "품질 (CQ)",
        ...
        "engine_first_run_body_rtx": "GPU 전용 모델 파일을 준비해야 합니다. 첫 실행 시 이는 정상입니다. TensorRT-RTX 컴파일은 보통 1분 이내에 완료됩니다.\n\n앱이 응답하지 않는 것처럼 보일 수 있습니다 — 닫지 마세요.\n\n모델 결과물은 캐시되어 이후 실행 시 자동으로 재사용됩니다.",
